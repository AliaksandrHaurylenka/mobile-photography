...
@section('content')
    ...
    <p>
        <ul class="list-inline">
            <li><a href="{{ route('admin.prices.index') }}" style="{{ request('show_deleted') == 1 ? '' : 'font-weight: 700' }}">@lang('quickadmin.qa_all')</a></li> |
            <li><a href="{{ route('admin.prices.index') }}?show_deleted=1" style="{{ request('show_deleted') == 1 ? 'font-weight: 700' : '' }}">@lang('quickadmin.qa_trash')</a></li>
        </ul>
    </p>
    @endcan
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body table-responsive">
            <table class="table table-bordered table-striped {{ count($prices) > 0 ? 'datatable' : '' }} @can('price_delete') @if ( request('show_deleted') != 1 ) dt-select @endif @endcan">
                <thead>
                    <tr>
                        @can('price_delete')
                    ...
                    @if (count($prices) > 0)
                        @foreach ($prices as $price)
                            <tr data-entry-id="{{ $price->id }}">
                                ...
                                <td>
                                    @can('price_view')
                                    <a href="{{ route('admin.prices.show',[$price->id]) }}" class="btn btn-xs btn-primary">@lang('quickadmin.qa_view')</a>
                                    @endcan
                                    @can('price_edit')
                                    <a href="{{ route('admin.prices.edit',[$price->id]) }}" class="btn btn-xs btn-info">@lang('quickadmin.qa_edit')</a>
                                    @endcan
                                    @can('price_delete')
{!! Form::open(array(
                                        'style' => 'display: inline-block;',
